...
document.addEventListener('click', function (e) {
  ...
  switch (action) {
    ...
    case 'open-symptom-checker':
      if (window.SymptomCheckerEngine) window.SymptomCheckerEngine.open();
      break;
    case 'close-symptom-checker':
      if (window.SymptomCheckerEngine) window.SymptomCheckerEngine.close();
      break;
    case 'open-review-modal':
      if (window.DoctorRatingsEngine) window.DoctorRatingsEngine.openModal(d.docId || 'dr-rajesh-sharma');
      break;
    case 'close-review-modal':
      if (window.DoctorRatingsEngine) window.DoctorRatingsEngine.closeModal();
      break;
...
const SymptomCheckerEngine = (function () {
  ...
  const SPECIALTY_MAP = {
    ...
    bones:   { icon: '🦴', dept: 'Orthopedics & Joint Surgery',     desc: 'Bone and joint discomfort should be assessed by our senior orthopedic surgeon for early diagnosis.', docId: 'orthopedics' },
    ...
    stomach: { icon: '🫁', dept: 'General Medicine / Gastro',       desc: 'Abdominal and digestive symptoms warrant thorough examination by our general physicians.', docId: 'general' },
    child:   { icon: '👶', dept: 'Pediatrics (Child Specialist)',   desc: 'Our senior pediatricians provide comprehensive child health care from newborns to teenagers.', docId: 'pediatrician' },
    womens:  { icon: '🌸', dept: 'Obstetrics & Gynecology',         desc: "Our gynecologists specialize in reproductive health, prenatal care, and women's wellness.", docId: 'gynecology' },
    ent:     { icon: '👂', dept: 'ENT (Ear, Nose & Throat)',        desc: 'Hearing, sinus, and throat conditions are handled by our specialist ENT surgeon.', docId: 'ent' },
    eye:     { icon: '👁️', dept: 'Ophthalmology & Eye Care',        desc: 'Vision problems and eye conditions are managed by our ophthalmology team.', docId: 'ophthalmology' },
    dental:  { icon: '🦷', dept: 'Dental & Oral Surgery',           desc: 'Our cosmetic and restorative dentists handle fillings, extractions, and implant procedures.', docId: 'dentist' },
    general: { icon: '🩺', dept: 'General Medicine',                desc: 'Our senior consultant physicians manage fever, fatigue, diabetes, and general health assessments.', docId: 'general' }
  };

  const EMERGENCY_RESULT = { icon: '🚨', dept: 'Emergency & Trauma (24/7)', desc: 'Your symptoms suggest URGENT evaluation. Please proceed to our 24/7 Emergency Bay or call Ambulance 108 immediately.', isEmergency: true };
  ...
  function showResult() {
    ...
    const iconEl = document.getElementById('symptom-result-icon');
    const deptEl = document.getElementById('symptom-result-dept');
    const descEl = document.getElementById('symptom-result-desc');
    if (iconEl) iconEl.textContent = result.icon;
    if (deptEl) deptEl.textContent = result.dept;
    ...
    if (bookBtn) {
      if (result.isEmergency) {
        bookBtn.textContent = '🚨 Call Emergency (108)';
